...
from maddpgAgent.buffer import BUFFER
from maddpgAgent.ddpgAgent import DDPGAGENT
...
class MADDPGAGENT:
    ...
    def __init__(self, dim_info, capacity, batch_size, actor_lr, critic_lr, action_bound):
        # 状态（全局观测）与所有智能体动作维度的和 即critic网络的输入维度
        ...
        for agent_id, (obs_dim, act_dim) in dim_info.items():
            # 每一个智能体都是一个DDPG智能体
            self.agents[agent_id] = DDPGAGENT(obs_dim, act_dim, global_obs_act_dim, actor_lr, critic_lr, self.device, action_bound[agent_id])
            # buffer均只是存储自己的观测与动作
    ...
    def learn(self, batch_size, gamma):
        for agent_id, agent in self.agents.items():
            ...
            a = list(next_obs.values())
            b = list(next_act.values())
